  ...
  lines.forEach(line => {
    ...
    if (line.startsWith('.')) {
      ...
      if (clean === 'flex') { target.style.display = 'flex'; }
        ...
        if (prop === 'height') target.style.height = numVal;
...
function initCanvasEngine(ast, container) {
  ...
  function emit(eventName, data) {
    for (const fn of (_eventListeners[eventName] || [])) {
      try { fn(data); } catch (e) { console.error('[UIScript]', e); }
    }
  }
  ...
  function measureNode(node, availWidth) {
    ...
    if (node.type === 'br') {
      ...
      node.width = availWidth;
      node.height = node.style.height || THEME.img.height || 160;
  ...
  function drawNode(node) {
    ...
    ctx.save();
    drawNodeBody(node, theme, pad, radius);
  }
  ...
  function drawNodeBodyInner(node, theme, pad, radius) {
    if (node.type === 'div' && (pad || radius)) {
      ...
      else { ctx.shadowColor = shadowFor(node); ctx.shadowBlur = 8; ctx.shadowOffsetY = 4; }
    ...
    if (node.type === 'option') {
      ...
      if (node.style.shadow) { applyShadow(node); }
      else { ctx.shadowColor = shadowFor(node); ctx.shadowBlur = 6; ctx.shadowOffsetY = 3; }
    ...
    if (node.type === 'input') {
      ...
      if (!node.style.shadow) { ctx.shadowBlur = node.focused ? 20 : 8; ctx.shadowOffsetY = node.focused ? 10 : 4; }
      fillSmoothRect(ctx, node.x, node.y, node.width, node.height, radius);
      ctx.strokeStyle = node.focused ? PRIMARY : mixHex((theme.border || '#D2D2D7'), '#8DB2FF', node.hoverAmount || 0);
      ctx.lineWidth = 1.5;
      strokeSmoothRect(ctx, node.x, node.y, node.width, node.height, radius);
      clearShadow();
    }
    ...
    if (node.type === 'radio') {
      ...
      const hover = node.hoverAmount || 0;
      if (!node.style.shadow) { ctx.shadowBlur = 5; ctx.shadowOffsetY = 2; }
    ...
    if (node.text !== undefined && node.type !== 'img') {
      ...
      if (node.type === 'button') {
        ...
        ctx.globalAlpha = textAlpha;
        ctx.fillText(text, node.x + pad, node.y + node.height / 2);
        ctx.globalAlpha = 1;
        if (node.focused) {
          const before = text.slice(0, node.caret);
        ...
        ctx.textAlign = 'left';
        ctx.globalAlpha = textAlpha;
        ctx.fillText(node.text, node.x + pad + 32, node.y + node.height / 2);
        ctx.globalAlpha = 1;
      } else {
  ...
  function drawDialog() {
    ...
    const viewW = canvas.clientWidth || container.clientWidth;
    const viewH = canvas.clientHeight || container.clientHeight;
  ...
  function render() {
    ...
    if (headerNode && headerHeight > 0) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        ctx.clip();
        ctx.drawImage(slices[i], 0, 0, cw, ch, 0, 0, viewW, headerHeight);
        ctx.restore();

        if (i < steps - 1) {
          for (let p = 0; p < blend; p++) {
            const alpha = (p + 1) / (blend + 1);
            const segY = y1 - blendHeight + (p * blendHeight / blend);
            const segH = blendHeight / blend;
            ctx.save();
            ctx.globalAlpha = 1 - alpha;
            ctx.beginPath();
            ctx.rect(0, segY, viewW, segH);
            ctx.clip();
            ctx.drawImage(slices[i], 0, 0, cw, ch, 0, 0, viewW, headerHeight);
            ctx.restore();
            ctx.save();
            ...
            ctx.rect(0, segY, viewW, segH);
            ctx.clip();
            ctx.drawImage(slices[i + 1], 0, 0, cw, ch, 0, 0, viewW, headerHeight);
            ctx.restore();
          }
        }
      }
  ...
  function updateAnimation(node, dtMs) {
    ...
    if (stepTimedAnimation(node.hoverAnim, target, dtMs, HOVER_MS)) animDirty = true;
    node.hoverAmount = node.hoverAnim.value;
    node.children.forEach((child) => {
      if (updateAnimation(child, dtMs)) animDirty = true;
    });
    return animDirty;
  }
  ...
  canvas.addEventListener('click', (e) => {
    ...
    if (hit) {
      ...
      if (hit.type === 'option') {
        ...
        emit('change', { id: hit.id || null, type: hit.type, value: hit.selected, text: hit.text });
      }
      if (hit.type === 'input') {
      ...
      if (hit.type === 'radio') {
        ...
        hit.selected = true;
        emit('change', { id: hit.id || null, type: 'radio', value: true, text: hit.text });
      }
      ...
      emit('click', { id: hit.id || null, type: hit.type, text: hit.text });
    }
    if (!hit) {
      ast.children.forEach(clearFocus);
    }
  ...
  function clearFocus(node) {
    if (node.focused) {
      node.focused = false;
      emit('change', { id: node.id || null, type: node.type, value: node.text });
    }
  ...
  canvas.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Backspace') {
      ...
      emit('input', { id: focused.id || null, text: focused.text });
      scheduleRender();
      return;
    }
    if (e.key === 'Delete') {
  ...
  const api = {
    on(eventName, fn) {
      if (_eventListeners[eventName]) _eventListeners[eventName].push(fn);
      return api;
    },
    off(eventName, fn) {
    ...
    getStyle(id, prop) {
      const node = findNodeById(ast, id);
      return node ? node.style[prop] : undefined;
    },
    ...
    setVisible(id, visible) {
      const node = findNodeById(ast, id);
      if (node) { node._hidden = !visible; scheduleRender(); }
      return api;
    },
    isVisible(id) {
      const node = findNodeById(ast, id);
      return node ? !node._hidden : false;
    },
    ...
    querySelector(selector) {
      const all = [];
      collectAllNodes(ast, all);
      return all.find(n => n.type === selector && n.id) || null;
    },
    querySelectorAll(selector) {
      const all = [];
      collectAllNodes(ast, all);
      return all.filter(n => n.type === selector && n.id);
    },
    destroy() {
      rafRunning = false;
